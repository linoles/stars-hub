...
bot.action(/lotery=(.+)/, async (ctx) => {
  ...
  if (!row.lotery.isActive) {
    ...
    if (Object.keys(row.lotery.currentWinners).length < row.lotery.winners) {
      ...
      lsendResults(`🎉 Лотерея окончена!\n<blockquote expandable>\t\t🥇 Победители: ${Object.entries(row.lotery.currentWinners).map((win) => `<a href="tg://user?id=${win[0]}">${win[1]} ${Object.keys(row.lotery.prizes).find((key: any) => row.lotery.prizes[key] === Number(win[0])) || "🎉"} (${win[0]})</a>`).join(", ")}</blockquote>`, row);
