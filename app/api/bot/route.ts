  ...
  try {
    ...
    sendLog(
      `🤝 Совершён перевод звёзд (${amount}⭐ - ${
        ...
        receiver.tgId
      }">${receiver.tgNick}</a> (#id${receiver.tgId})\nБаланс отправителя: ${
        sender.stars - amount
      }⭐\nБаланс получателя: ${receiver.stars + amount * 0.95}⭐ #transfer`
    );
  } catch (error) {
...
bot.on("pre_checkout_query", async (ctx) => {
  try {
    ...
    if (user) {
      const newStars = user.stars + ctx.update.pre_checkout_query.total_amount;
      await supabase
        .from("users")
