...
import admBottomUp from "./admBottomUp";
...
const procAdmBottomUp = async (curUser: any) => {
  const form = document.getElementById("admBottomUpForm") as HTMLFormElement;
  ...
  if (form) {
    ...
    if (receiver.stars < Number(amount)) {
      alert("❌ Упс, у него столько нету!");
      return;
    }
    
    alert("✅ Списание успешно выполнено!");

    usernameInput.value = "";
    amountInput.value = "";

    admBottomUp({ sender: curUser, receiver: (await receiver), amount: amount });
  } else {
    alert("❌ Произошла ошибка!");
  }
  return;
...
export default function AdmInterface({ curMode, curUser }: { curMode: string, curUser: any }) {
  switch (curMode) {
    ...
    case "bottom_up":
      return (
        <div className="flex flex-col justify-center items-center mt-4">
          <p className={"text-white font-extrabold text-2xl w-full h-fit text-center " + inter.className}>СПИСАНИЕ СРЕДСТВ</p>
          <form action="none" id="admBottomUpForm" onSubmit={(e) => e.preventDefault()}>
            <div className="w-[calc(100%-20px)] h-fit flex flex-row items-center justify-start mt-2 border-b ml-[10px] mr-[10px] border-b-gray-800">
              <p className={"text-casino-gold/40 font-extrabold text-lg w-full h-fit text-start " + inter.className}>✍ @USERNAME</p>
              <input name="username" type="text" className={"bg-transparent text-gray-700 border-0 outline-0 font-semibold text-sm w-full h-fit text-start " + inter.className} placeholder="Пострадавший @user 👤" />
            </div>
            <div className="w-[calc(100%-20px)] h-fit flex flex-row items-center justify-start mt-2 border-b ml-[10px] mr-[10px] border-b-gray-800">
              <p className={"text-casino-gold/40 font-extrabold text-lg w-6/12 h-fit text-start " + inter.className}>✍ СУММА</p>
              <input name="amount" type="number" className={"bg-transparent text-gray-700 border-0 outline-0 font-semibold text-sm w-full h-fit text-start " + inter.className} placeholder="Число SCoins 🪙" />
          ...
          <div className="w-[calc(100%-20px)] h-fit flex flex-row items-center justify-start mt-2 ml-[10px] mr-[10px]" onClick={() => procAdmBottomUp(curUser)}>
            <div className={"font-extrabold text-3xl text-white w-full h-fit text-center bg-casino-gold/80 hover:bg-casino-gold/40 cursor-pointer py-4 rounded-4xl " + inter.className}>СПИСАТЬ</div>
          </div>
        </div>
      );
    default:
